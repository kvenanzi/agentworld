...
export const digestRoute = new Hono<AppEnv>().get("/", async (c) => {
  ...
  const caretakerHealth: Record<string, { last_event_id: number; updated_at: number } | null> = {};
  for (const persona of WORLD.caretakers) {
    const s = await getCaretakerState(db, persona);
    caretakerHealth[persona] = s ? { last_event_id: s.last_event_id, updated_at: s.updated_at } : null;
  }
  return c.json({
